...
const Timer = () => {
    ...
    //-----------------------------------------------------------------------------------------------------------------
    //                                                      // Pause the timer when the app is close
    useEffect(() => {
        return () => {
            funPauseTimer();
        };
    ...
    //-----------------------------------------------------------------------------------------------------------------
    //                                                      // Set the task as completed when timer is finished
    useEffect(() => {
        if (state.selectedTask && time >= state.selectedTask.taskFinalDuration) {
            funCompleteTask();
        }
    }, [time, state.selectedTask]);
    ...
    const funCompleteTask = () => {
        funPauseTimer();
        if (state.selectedTask) {
            //                                              // Complete the task in state
            dispatch({
                type: 'COMPLETE_TASK',
                payload: {
                    taskId: state.selectedTask.taskId,
                    taskCompletedDuration: time,
                },
            });
            //                                              // Clean the selected task in state
            dispatch({
    ...
    return (
        <Box id="tasks-timer">
            ...
            >
                {
                    // Context message when there is not a seletec task
                    ...
                    : (
                        <>
                            ...
                            <div className='timer-container__info'>
                                <Typography variant='h5' className='title fw-bolder tt-uppercase'>
                                    { state.selectedTask.taskTitle }
                                </Typography>
                                <Typography variant='h6' className='fc-blue-1 fst-italic'>
                                    Duración: {Math.floor(state.selectedTask.taskFinalDuration / 3600)}h:
                                    {Math.floor((state.selectedTask.taskFinalDuration % 3600) / 60)}m:
                                    {state.selectedTask.taskFinalDuration % 60}s
                                </Typography>
                            </div>
